...
export default function SequenceButtons() {
  ...
  const { data, isSuccess } = useQuery(
    ...
    async () =>
      await getData<Sequence>(
        `courses/${router.query.course}/module_item_sequence?asset_type=ModuleItem&asset_id=${router.query.moduleItem}`
      )
  )
  ...
  switch (data.type) {
    case 'Assignment':
      queryClient.prefetchQuery(
        ...
        async () =>
          getData<Assignment>(`courses/${router.query.course}/assignments/${data.content_id}`)
      )
      break
    case 'File':
      queryClient.prefetchQuery(
        ['courses', router.query.course, 'file', data.content_id.toString()],
        async () => await getData<File>(`courses/${router.query.course}/files/${data.content_id}`)
      )
      break
    case 'Page':
      queryClient.prefetchQuery(
        ['courses', router.query.course, 'pages', data.page_url.toString()],
        async () => getData<Page>(`courses/${router.query.course}/pages/${data.page_url}`)
      )
      break
  }
}
